module Semtrace
  ...
  class Decomposer
    ...
    # Decomposes using inner product search.
    # Requires the store to have an inner product HNSW index.
    def decompose_ip(
      target : Array(Float32) | Slice(Float32),
      epsilon : Float32 = 0.01_f32,
      max_steps : Int32 = 20,
      ...
      raise "Inner product index not available" unless @store.has_ip_index?
      ...
      max_steps.times do
        ...
        candidates = @store.search_ip(residual, k: lookahead)
        break if candidates.empty?
